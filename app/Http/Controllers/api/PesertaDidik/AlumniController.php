...
class AlumniController extends Controller
{
    ...
    {
        try {
            // 1) Sub‐query: tanggal_keluar alumni terakhir per peserta (pelajar)
            $pelajarLast = DB::table('pelajar')
                ->select('id_peserta_didik', DB::raw('MAX(tanggal_keluar) AS max_tanggal_keluar'))
                ->where('status', 'alumni')
                ->groupBy('id_peserta_didik');

            // 2) Sub‐query: tanggal_keluar alumni terakhir per peserta (santri)
            $santriLast = DB::table('santri')
                ->select('id_peserta_didik', DB::raw('MAX(tanggal_keluar) AS max_tanggal_keluar'))
                ->where('status', 'alumni')
                ->groupBy('id_peserta_didik');

            // 3) Sub‐query: tanggal_keluar riwayat_pendidikan alumni terakhir per peserta
            $riwayatLast = DB::table('riwayat_pendidikan')
                ->select('id_peserta_didik', DB::raw('MAX(tanggal_keluar) AS max_tanggal_keluar'))
                ->where('status', 'alumni')
                ->groupBy('id_peserta_didik');

            // 4) Sub‐query: domisili alumni terakhir per peserta
            $domisiliLast = DB::table('riwayat_domisili')
                ->select('id_peserta_didik', DB::raw('MAX(id) AS last_id'))
                ->where('status', 'alumni')
                ->groupBy('id_peserta_didik');

            // 5) Sub‐query: warga_pesantren terakhir per biodata
            $wpLast = DB::table('warga_pesantren')
                ->select('id_biodata', DB::raw('MAX(id) AS last_id'))
                ->where('status', true)
                ->groupBy('id_biodata');

            // 6) Sub‐query: ID Pas Foto terakhir per biodata
            ...
            $query = DB::table('peserta_didik as pd')
                ...
                // Pelajar alumni terakhir
                ->leftJoinSub($pelajarLast, 'lp', fn($j) => $j->on('lp.id_peserta_didik', '=', 'pd.id'))
                ->leftJoin('pelajar as p', fn($j) => $j->on('p.id_peserta_didik', '=', 'lp.id_peserta_didik')->on('p.tanggal_keluar', '=', 'lp.max_tanggal_keluar'))

                // Santri alumni terakhir
                ->leftJoinSub($santriLast, 'ls', fn($j) => $j->on('ls.id_peserta_didik', '=', 'pd.id'))
                ->leftJoin('santri as s', fn($j) => $j->on('s.id_peserta_didik', '=', 'ls.id_peserta_didik')->on('s.tanggal_keluar', '=', 'ls.max_tanggal_keluar'))

                // Riwayat_pendidikan alumni terakhir → lembaga
                ->leftJoinSub($riwayatLast, 'lr', fn($j) => $j->on('lr.id_peserta_didik', '=', 'pd.id'))
                ->leftJoin('riwayat_pendidikan as rp', fn($j) => $j->on('rp.id_peserta_didik', '=', 'lr.id_peserta_didik')->on('rp.tanggal_keluar', '=', 'lr.max_tanggal_keluar'))
                ->leftJoin('lembaga as l', 'rp.id_lembaga', '=', 'l.id')

                // Domisili alumni terakhir → wilayah/blok/kamar
                ->leftJoinSub($domisiliLast, 'ld', fn($j) => $j->on('ld.id_peserta_didik', '=', 'pd.id'))
                ->leftJoin('riwayat_domisili as rd', 'rd.id', '=', 'ld.last_id')
                ->leftJoin('wilayah as w', 'rd.id_wilayah', '=', 'w.id')
                ->leftJoin('blok as bl', 'rd.id_blok', '=', 'bl.id')
                ->leftJoin('kamar as km', 'rd.id_kamar', '=', 'km.id')

                // Warga Pesantren terakhir
                ->leftJoinSub($wpLast, 'lw', fn($j) => $j->on('lw.id_biodata', '=', 'b.id'))
                ->leftJoin('warga_pesantren as wp', 'wp.id', '=', 'lw.last_id')

                // Kabupaten & Pas Foto
                ->leftJoin('kabupaten as kb', 'kb.id', '=', 'b.id_kabupaten')
                ->leftJoinSub($fotoLast, 'lf', fn($j) => $j->on('lf.id_biodata', '=', 'b.id'))
                ->leftJoin('berkas as br', 'br.id', '=', 'lf.last_id')

                // Filter: hanya alumni pelajar atau santri
