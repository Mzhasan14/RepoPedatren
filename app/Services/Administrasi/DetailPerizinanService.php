...
{
    public function getDetailPerizinan(string $idPerizinan): array
    {
        // Pemohon Izin
        $biodata = DB::table('santri as s')
            ...
            ->select([
                ...
                'pr.tanggal_mulai',
                'pr.tanggal_akhir',
                // Masukkan raw expression tanpa implode
                DB::raw("
                  CASE
        ...
        // Subquery untuk id jenis “Bukti Izin”
        $jenisIzinId = DB::table('jenis_berkas')
            ->where('nama_jenis_berkas', 'Bukti Izin')
            ->value('id');

        $berkas = DB::table('perizinan as pr')
            ->join('santri as s', 's.id', 'pr.santri_id')
            ->join('biodata as b', 's.biodata_id', 'b.id')
            ->join('berkas as br', function ($j) use ($jenisIzinId) {
                $j->on('b.id', 'br.biodata_id')
                    ->where('br.jenis_berkas_id', $jenisIzinId)
                    ->whereColumn('br.created_at', '>', 'pr.created_at');
            })
            ->where('pr.id', $idPerizinan)
            ->orderBy('br.created_at', 'asc')        // urut dari terawal setelah izin
            ->limit(2)                              // ambil maksimal 2
            ->selectRaw("COALESCE(br.file_path, 'default.jpg') as file_path")
            ->get();
